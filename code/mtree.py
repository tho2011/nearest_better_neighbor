...
class _Node(_IndexItem):
    ...
    def check_max_capacity(self, mtree):
        if len(self.children) > mtree.max_node_capacity:
            ...
            data_objects = frozenset(iter(self.children.keys()))
...
class MTree(object):
    ...
    def get_nearest(self, query_data, rad =_INFINITY, limit=_INFINITY):
        """
        ...
        while pending_queue:
            ...
            for child in iter(node.children.values()):
            #for child in node.children.itervalues():
                #print('x')
                if abs(pending.distance - child.distance_to_parent) - child.radius <= rad:
                    child_distance = self.distance_function(query_data, child.data)
                    child_min_distance = max(child_distance - child.radius, 0)
                    if child_min_distance <= rad:
